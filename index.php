...
//теперь функция рекурсивно формирует  массив элементов с индексом $index, найденных во всех вложенных масивах
//глобальные переменные не юзаются

function someElement(array $arr, int $index): array
{
    $fullArr = array();
    ...
    foreach ($arr as $k=>$v)//бежим циклом, ключ нужен, чтоб залить в результирующий массив
    {
        ...
        {
            $fullArr[]=someElement($v, $index);//рекурсия и + к имеющемуся массиву
        }
        else
        {
            if ($i==$index) //если текущий элемент имеет порядковый номер, равный переданному
            {
                $fullArr[]=[$k=>$v];//добавляю новюую пару ключ-значение в имеющийся массив
            }
        }
        $i++;//следующий индекс
    }
 return $fullArr;//сформированный массив уходит в выдачу
}

echo '
## процедура отдаёт массив с искомыми элементами, но туда может попадать и подходящий элемент из основного массива </br>
## последовательность выданных элементов сильно зависит от структуры массива. </br>
...
## глобальные переменные не юзаются </br>
';
...
for ($indexPos=0; $indexPos<7; $indexPos++)
{
   echo "</br> выводим данные для искомого элемента с порядковым номером = $indexPos : </br></br>";
    ...
    for ($i = 1; $i < 4; $i++)
    {
        ...
        echo 'для массива ' . 'ar' . $i . 'dim :' . json_encode(someElement(${$arr}, $indexPos)) . '</br>';
    }
    //с масисвами $as1dim, $as2dim, $as3dim
    for ($i = 1; $i < 4; $i++)
    {
        $arr = 'as' . $i . 'dim';//имя переменной переменной для внутрицикловой обработки
        echo 'для массива ' . 'as' . $i . 'dim :' . json_encode(someElement(${$arr}, $indexPos)) . '</br>';
    }
